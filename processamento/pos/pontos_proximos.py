...
SAIDAS = [
    f'/home/cassianodragao/scripts/analise/{CENARIO_0}_{CENARIO_1}/{CENARIO_0}_pts_proximos_{NUMERO}_novo.csv',
    f'/home/cassianodragao/scripts/analise/{CENARIO_0}_{CENARIO_1}/{CENARIO_1}_pts_proximos_{NUMERO}_novo.csv'
]
############################################################################################
...
# olhar os boarding times, e ver se o destino dos links esta a 500m do metro
board0_uni = board0.drop_duplicates(subset="link_chegou_ponto", keep="first")
board1_uni = board1.drop_duplicates(subset="link_chegou_ponto", keep="first")

links_prox_0 = []
for _, row in board0_uni.iterrows():
    node_to = edges0.loc[edges0['id']==row['link_chegou_ponto'], 'to_'].iat[0]
    node_row = nodes0.loc[nodes0['id']==node_to]
    node_pt = (node_row.iloc[0]['lat'], node_row.iloc[0]['lon'])
    for st in estacoes_interesse:
        if geodesic(st, node_pt).meters < 500:
            links_prox_0.append(row['link_chegou_ponto'])
            break
links_prox_1 = []
for _, row in board1_uni.iterrows():
    node_to = edges1.loc[edges1['id']==row['link_chegou_ponto'], 'to_'].iat[0]
    node_row = nodes1.loc[nodes1['id']==node_to]
    node_pt = (node_row.iloc[0]['lat'], node_row.iloc[0]['lon'])
    for st in estacoes_interesse:
        if geodesic(st, node_pt).meters < 500:
            links_prox_1.append(row['link_chegou_ponto'])
            break

b0 = board0.loc[board0['link_chegou_ponto'].isin(links_prox_0)]
b0.to_csv(SAIDAS[0], index=False)
b1 = board1.loc[board0['link_chegou_ponto'].isin(links_prox_1)]
